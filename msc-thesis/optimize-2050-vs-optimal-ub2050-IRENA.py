...
target_IC = ic_2030.sel(index='IRENA 2050').Europe.values*1000
...
fig = ax.get_figure()
fig.savefig(data_folder / 'fig/optimize_2050_all_ub2050_irena.png')
...
df_ic = pd.DataFrame((data_ic), columns=['Planed IC 2030', 'With total IC (planed for 2050) as constraint', 'With total production (planed for 2030) as constraint', 'With total IC and production (planed for 2030) as constraint'], index=country)
df_ic.to_excel(data_folder / 'ic_new_2050_irena.xlsx')
...
f.savefig(data_folder / 'fig/new_ic_destribution_2050_irena.png')
