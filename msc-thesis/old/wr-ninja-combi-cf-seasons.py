# -*- coding: utf-8 -*-
"""
Created on Wed Sep 16 09:50:10 2020
...
f_out1 = data_folder / 'results/mean_wr_country_c7'
f_out2 = data_folder / 'results/relative_mean_wr_country_c7'
...
def wr_cf(ninja):
    ...
    return mean_wr_country, relative_mean_wr_country




ninja.time = pd.to_datetime(ninja.time)

#calculate DJF
DJF = ninja[(ninja.time.dt.month==12)|(ninja.time.dt.month==1)|(ninja.time.dt.month==2)]
DJF = DJF.reset_index()
DJF.time = DJF.time.astype('str')
DJF = DJF.drop(columns='index')
DJF_mean_wr_country, DJF_relative_mean_wr_country = wr_cf(DJF)
#save results as csv
DJF_mean_wr_country.to_csv(str(f_out1) + '_DJF.csv')
DJF_relative_mean_wr_country.to_csv(str(f_out2) + '_DJF.csv')


#calculate MAM
MAM = ninja[(ninja.time.dt.month==3)|(ninja.time.dt.month==4)|(ninja.time.dt.month==5)]
MAM = MAM.reset_index()
MAM.time = MAM.time.astype('str')
MAM = MAM.drop(columns='index')
MAM_mean_wr_country, MAM_relative_mean_wr_country = wr_cf(MAM)
#save results as csv
MAM_mean_wr_country.to_csv(str(f_out1) + '_MAM.csv')
MAM_relative_mean_wr_country.to_csv(str(f_out2) + '_MAM.csv')

#calculate JJA
JJA = ninja[(ninja.time.dt.month==6)|(ninja.time.dt.month==7)|(ninja.time.dt.month==8)]
JJA = JJA.reset_index()
JJA.time = JJA.time.astype('str')
JJA = JJA.drop(columns='index')
JJA_mean_wr_country, JJA_relative_mean_wr_country = wr_cf(JJA)
#save results as csv
JJA_mean_wr_country.to_csv(str(f_out1) + '_JJA.csv')
JJA_relative_mean_wr_country.to_csv(str(f_out2) + '_JJA.csv')

#calculate SON
SON = ninja[(ninja.time.dt.month==9)|(ninja.time.dt.month==10)|(ninja.time.dt.month==11)]
SON = SON.reset_index()
SON.time = SON.time.astype('str')
SON = SON.drop(columns='index')
SON_mean_wr_country, SON_relative_mean_wr_country = wr_cf(SON)
#save results as csv
SON_mean_wr_country.to_csv(str(f_out1) + '_SON.csv')
SON_relative_mean_wr_country.to_csv(str(f_out2) + '_SON.csv')
