...
#IC 2030
target_IC = ic_reduced_2030.to_array().sum()/2
b_tot_IC = np.append(b, target_IC)      


#####WITH TOT PRODUCTION AS CONSTRAINT
#Define total production
P = (mean_season.mean().to_array() * ic_reduced_2030.to_array()).sum()*1.5
...
W_both = np.ones(b_both.shape)*250
W_both[-2]=1
W_both[-1]=55
...
data_var = np.c_[current_state,var_both]
...
df_var = pd.DataFrame((data_var), columns=['Planed IC 2030', 'With total IC and production (planed for 2030) as constraint'])
for i in range(0,8):
...
data_ic = np.c_[ic_reduced_2030.to_array().values, res_both.x]
df_ic = pd.DataFrame((data_ic), columns=['Planed IC 2030', 'With total IC and production (planed for 2030) as constraint'], index=country)
