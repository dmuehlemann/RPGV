...
ic_min_load = (load.drop_vars('UA')/ (365*24)) / cf *0.3
...
var_2050 = 'Variability with installed PV capacity upscaled for 2050' #'Variability with installed PV capacity planned for 2050'
var_prod = 'Variability of scenario variability'
var_cost = 'Variability of scenario low costs'
var_autarky = 'Variability of scenario autarky'
# IC_2019_name = 'Installed PV capacity 2019'
IC_2050_name = 'Installed PV capacity upscaled for 2050'
IC_prod_name = 'Installed PV capacity of scenario variability'
IC_cost_name = 'Installed PV capacity of scenario low costs'
IC_autarky_name = 'Installed PV capacity of scenario autarky'                     
...
P = (mean_season.mean().to_array() * ic_reduced_2050.to_array()).sum()*1.5
# P_cost = (mean_season.mean().to_array() * ic_reduced_2050.to_array()).sum()*1.5
...
# df_var = pd.DataFrame((data_var), columns=['Planned IC 2050', 'With total IC (planned for 2050) as constraint', 'With total production (Planned for 2050) as constraint', 'With total IC and production (Planned for 2050) as constraint'])
...
# df_var_frequency = pd.DataFrame((data_var_fre), columns=['Variability with installed PV capacity 2019', 'Variability with installed PV capacity planned for 2050', 'Variability with installed PV capacity and production (2050) as constraint (S1)'])
# for i in range(0,8):
...
#with total Planned IC / power production for 2050 as constraint for every season
# ax_var.set_title('Optimized IC distribution (with installed PV capacity planned for 2050)', fontsize=24, pad=20)
...
# data_ic = np.c_[ic_reduced_2050.to_array().values, res_tot_IC.x, res_tot_P.x, res_S1.x]
data_ic = np.c_[ic_reduced_2050.to_array().values, res_prod.x, res_cost.x, res_autarky.x]
df_ic = pd.DataFrame((data_ic), columns=[IC_2050_name, IC_prod_name, IC_cost_name, IC_autarky_name], index=country)
df_ic.to_excel(data_folder / str(project + 'new-ic.xlsx'))
#ic minus lower bound
df_ic_lb = round((df_ic[[IC_2050_name, IC_prod_name, IC_cost_name, IC_autarky_name]].transpose() -lb).transpose())
df_ic_ub = round((df_ic[[IC_2050_name, IC_prod_name, IC_cost_name, IC_autarky_name]].transpose() -ub).transpose())
...
for i in ic_plotting:
    if n== 0:
        ...
        i.dropna().plot(ax=ax[r,c], column=i.columns[3], cmap=cmap,edgecolor='black',linewidth=0.1,
                                  vmax=vmax, vmin=vmin,
                                  legend=True, 
        ...
        ax[r, c].set_xlim(left=-13, right=35)
        ax[r, c].set_ylim(bottom=34, top=70) 
        ax[r, c].set_title(r"$\bf{" + i.columns[3].replace(' ', '~') + "}$" +
                        '\n Total installed PV capacity (GW): ' + str((tot_IC[n+1]/1000).round(1)) + 
                        '\n Total mean PV production (GW): ' + str((tot_P[n+1].values/1000).round(1)) +
                        '\n Total mean variability (GW): ' + str((tot_var[n]/1000).round(1))+ 
                        '\n Total max variability (GW): ' + str((max[5*(n+1)-1]).round(1)), 
                        fontsize=7)
        ax[r, c].set_axis_off()
        n = n +1
        c = c + 1
        ...
        for column in df_ic_ub:
            if column == i.columns[3]:
                for cc in (df_ic_ub[column].where(df_ic_ub[column]>-1)).dropna().index:
                    ...
                    i.where(i.country_code==cc).plot(ax=ax[r,c],column=i.columns[3], cmap=cmap, edgecolor='black',linewidth=0.1,
...
leg1 = ax[0,0].get_figure().get_axes()[4]
leg1.set_position([0.37,0.05,0.3,0.1])
leg1.set_xlabel('Total installed PV capacity (in GW)', fontsize=7)
ticklabs = leg1.get_xticklabels()
leg1.set_xticklabels(ticklabs, fontsize=7)
#move subplot
pos2 = [ax[0,0].get_position().x0, ax[0,1].get_position().y0, ax[0,1].get_position().width, ax[0,1].get_position().height]
ax[0,0].set_position(pos2)
pos4 = [ax[0,0].get_position().x0, ax[0,1].get_position().y0, ax[0,1].get_position().width, ax[0,1].get_position().height]
ax[0,0].set_position(pos4)
...
for i in ic_lb_plotting:
    if n == 0:
        ...
        # cax = divider.append_axes("bottom", size="5%", pad=0.4)
        ma = i.dropna().plot(ax=ax[r, c], column=i.columns[3], cmap=cmap, edgecolor='black', linewidth=0.1,
                                  vmax=vmax, vmin=vmin,
                                  legend=True,
                                  legend_kwds={'orientation': "horizontal",}
        ...
        ax[r,c].set_xlim(left=-13, right=35)
        ax[r,c].set_ylim(bottom=34, top=70) 
        ax[r,c].set_title(r"$\bf{" + i.columns[3].replace(' ', '~').replace('Installed', 'Additional~installed')  + "}$" +
                        '\n Additional installed PV capacity (GW): ' + str(((tot_IC[n+1]-tot_IC[0])/1000).round(1)) + 
                        '\n Mean PV production (GW): ' + str(((tot_P[n+1]).values/1000).round(1)) +
                        '\n Total mean variability (GW): ' + str((tot_var[n]/1000).round(1)) +
                        '\n Total max variability (GW): ' + str((max[5*(n+1)-1]).round(1)), 
                        fontsize=7)
        ax[r,c].set_axis_off()
...
# Move legend to rigt place
leg1 = ax[0,0].get_figure().get_axes()[4]
leg1.set_position([0.37,0.01,0.3,0.1])
